...
Use the Browser skill's `tab.playwright` API for all Seller Central interactions. Do not use shell-installed Playwright, a separate local Amazon-label project, browser profiles, cookies, saved passwords, or direct HTTP requests to Seller Central. Work only in the user's existing interactive session; do not request, handle, or store credentials, MFA codes, or session data.
...
- Wait for a relevant page state or dialog instead of fixed delays. Use a browser download event only as an additional signal; Seller Central can hand the file to Chrome without emitting that event. For Seller Central controls that update derived totals only after focus leaves a field, explicitly commit the field change with `Enter`, `Tab`, or a click outside the field, then verify the updated page state before continuing.
...
- Accept pasted SKUs, `SKU,quantity` lines, a CSV/XLSX column pair, or a plain-text file. Preserve the seller SKU exactly except for surrounding whitespace, ignore blank rows, and use quantity `1` only when omitted.
- Interpret quantity as the number of labels inside one PDF for that SKU, not as a request for multiple separate PDFs. For example, `SKU,30` should produce one PDF containing 30 labels for that SKU when Seller Central supports that output.
...
- Default to download-only. Do not print to a physical printer unless the user explicitly requests printing and confirms the loaded label stock matches Seller Central's selected paper/sticker type. Never scale Amazon's PDF.
- Unless the user specifies another destination, collect only the current run's matched PDFs into `FNSKU labels/current/` in the current workspace. Create `FNSKU labels/` and `FNSKU labels/current/` first if either folder is missing. Keep the originals in Chrome Downloads.
...
3. Navigate with Playwright to Seller Central and confirm that the correct marketplace is active. If sign-in or MFA is required, ask the user to complete it in the selected browser and continue only after they say it is ready. Never ask for, handle, or store credentials or MFA codes.
...
6. On each label page, verify that the row contains the intended seller SKU and that the label count and paper format match the agreed choices. Do not assume that the label page's default is correct.
   - When changing the label count, fill the row's number input, then commit the change by pressing `Enter` or `Tab` on that input, or by clicking a neutral visible area outside the field.
   - After committing the change, verify both the input value and Seller Central's displayed total/summary quantity. If the input value and the displayed total disagree, do not click `Print Item Labels`; refocus the field, recommit the quantity once, and verify again from a fresh DOM snapshot.
   - If the displayed total still does not match the intended label count, stop before the final print click and hand the page to the user with status `error` or `needs user action`. A stale total means Seller Central may not have accepted the requested quantity.
...
13. Keep a result table with seller SKU, resolved FNSKU, product identifier/title, paper format, label count, source download filename, copied workspace path, and status (`downloaded`, `needs user download check`, `not found`, `ambiguous`, `error`). Continue to the next SKU after a SKU-specific failure when doing so will not duplicate a final label request or create an unsafe side effect.
...
- Treat the saved PDF as the source of truth for the final deliverable. When possible, verify that the copied PDF opens and its page count/layout is plausible for the requested quantity and selected paper format.
- Report successful downloads and every exception. Never silently skip a requested SKU.
- If a SKU-specific failure leaves visible evidence such as an error page, alert, or failed label page, preserve the live tab as a handoff or capture a screenshot when helpful, and report where the user can inspect it.
- On a timeout, stale quantity total, or browser interruption, do not resubmit a label request. Reconnect to the existing `printitemlabel` tab, inspect its current state, and preserve it as a handoff for the user if the outcome remains uncertain. Do not use an alert or missing download event as a reason to issue a second print click.
